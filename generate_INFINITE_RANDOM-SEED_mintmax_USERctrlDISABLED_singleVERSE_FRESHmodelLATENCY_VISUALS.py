...
parser.add_argument('--minlines', type=float, default=6,
                    help='MINimum lines per verse')
parser.add_argument('--maxlines', type=float, default=8,
                    help='MAX lines per verse')


parser.add_argument('--minwords', type=float, default=12,
                    help='MINimum chars per lines ')
parser.add_argument('--maxwords', type=float, default=32,
...
while(True):
   ...
   #print ("\n\n\n\t\t~ + ~\n\n")
    ...
    with open(args.outf, 'a') as outf:
        ...
        words = words.replace("Http://alt.venus.co.uk/weed/writings/poems/agh.htm","")

        words = words.replace("niggers","slaves")
        ...
        number_of_lines=randint(minlines,maxlines)

        w2=""
        li=" * "
        cnt=0

        # REPEATING WORD ON SCREEN #
        for w in words.split(" "):
            
            for x in range(args.minwords,args.maxwords):
                w2 = w2 +" "+w
